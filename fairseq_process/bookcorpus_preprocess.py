...
def SplitWords(s):
    words = []
    word_count = 0
    for m in re.finditer(r"(^|[\s'\-`])(([a-zA-Z]+\w*[a-zA-Z]*)|([0-9]+)|([.,?!]+))|([.,?!]+)", s):
        words.append(m.group(2) if m.group(2) is not None else m.group(0))
        word_count += 1
    if None in words:
        print(s)
        print(words)
        for m in re.finditer(r"(^|[\s'\-`])(([a-zA-Z]+\w*[a-zA-Z]*)|([0-9]+)|([.,?!]+))|([.,?!]+)", s):
            print(m)
            print("first:{}, second:{}, thrid:{}".format(m.group(0), m.group(1), m.group(2)))
        exit(-1)
    ...
    with(open(in_fn,'r')) as in_file:
        for s in in_file:
            if s in sentences:
                ...
                sentences.add(s)
    print('Total amount of duplicates : {}'.format(num_duplicates))
...
Concatenate('/data/scratch/panso014/language_modeling/bookcorpus/books_in_sentences.txt', \
    '/data/datasets/lm/bookcorpus/books_large_p1.txt', '/data/datasets/lm/bookcorpus/books_large_p2.txt')
AnalyseDuplicates('/data/scratch/panso014/language_modeling/bookcorpus/books_in_sentences.txt')
sentences = set()
SentencesToDistinct('/data/scratch/panso014/language_modeling/bookcorpus/books_in_sentences.txt', \
    '/data/scratch/panso014/language_modeling/bookcorpus/distinct_sentences.txt', sentences)
