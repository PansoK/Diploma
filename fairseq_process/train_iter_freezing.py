...
def main(cfg: FairseqConfig) -> None:
    ...
    ##########################
    if cfg.model.parallel_ffns_inference_training_type not in ["together", "sequentially", "iteratively"]:
        raise ValueError("Possible values for parallel_ffns_inference_training_type are 'together', 'sequentially' and 'iteratively'")
    if cfg.model.attention_heads_inference_training_type not in ["together", "sequentially", "iteratively"]:
        raise ValueError("Possible values for attention_heads_inference_training_type are 'together', 'sequentially' and 'iteratively'")

    if cfg.model.parallel_ffns_inference_training_type == "sequentially":
        set_ffn_inf_grad(cfg, model, value=False)

    if cfg.model.attention_heads_inference_training_type == "sequentially":
        set_head_inf_grad(cfg, model, value=False)
    ##########################
    ...
    while epoch_itr.next_epoch_idx <= max_epoch:
        ...
        ##########################
        # Assumption: If both set to 'sequentially' they are trained together. If one is set to 'together' its training stops with the rest
        if (cfg.model.parallel_ffns_inference_training_type == "sequentially" or cfg.model.attention_heads_inference_training_type == "sequentially") \
            and epoch_itr.next_epoch_idx == model_training_epochs+1:
            for p in model.parameters():
                p.requires_grad = False
            if cfg.model.parallel_ffns_inference_training_type == "sequentially":
                set_ffn_inf_grad(cfg, model, value=True)
            if cfg.model.attention_heads_inference_training_type == "sequentially":
                set_head_inf_grad(cfg, model, value=True)
        elif (cfg.model.parallel_ffns_inference_training_type == "iteratively" or cfg.model.attention_heads_inference_training_type == "iteratively"):
            train_inf = (epoch_itr.next_epoch_idx%2 == 0)  # boolean, True if training inference modules and not the rest, else False
            for p in model.parameters():
                p.requires_grad = not train_inf
            if cfg.model.parallel_ffns_inference_training_type == "iteratively":
                set_ffn_inf_grad(cfg, model, value=train_inf)
            if cfg.model.attention_heads_inference_training_type == "iteratively":
                set_head_inf_grad(cfg, model, value=train_inf)
        ##########################
