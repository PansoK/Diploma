...
@dataclass
class CompetitiveConfig(FairseqDataclass):
    parallel_ffns: int = field(default=1, metadata={"help": "number of parallel ffns per layer"})
    parallel_ffns_signature_dim: int = field(default=24, metadata={"help": "dimension of inference space for parallel ffns"})
    parallel_ffns_inference_mlp_hidden: int = field(default=128, metadata={"help": "hidden dimension of mlp that will produce inference vectors"})
    parallel_ffns_inference_mlp_activ_dropout: float = field(
        default=0.0,
        metadata={
    ...
    attention_heads_inference_mlp_activ_dropout: float = field(
        default=0.0,
        metadata={
            "help": "dropout probability after activation in inference FFN.",
            "alias": "--relu-dropout",
...
@dataclass
class EncDecCompConfig(FairseqDataclass):
...
@dataclass
class DecoderCompConfig(EncDecCompConfig):
    ...
    competitive: CompetitiveConfig = CompetitiveConfig()
    competitivecrossattn: CompetitiveCrossAttnConfig = CompetitiveCrossAttnConfig()
...
@dataclass
class TransformerCompConfig(FairseqDataclass):
    ...
    encoder: EncDecCompConfig = EncDecCompConfig()
            ...
            for fld in fields(cls):
                ...
                if fld.name == "decoder":
                    if safe_hasattr(args, "decoder"):
                        ...
                        seen.add("decoder")
                        config.decoder = DecoderCompConfig(**args.decoder)
                        config.decoder.competitive = CompetitiveConfig(**args.decoder.competitive)
                        config.decoder.competitivecrossattn = CompetitiveCrossAttnConfig(**args.decoder.competitivecrossattn)
                    else:
                        config.decoder = cls._copy_keys(
                            args, DecoderCompConfig, "decoder", seen
                        )
                        config.decoder.competitive = cls._copy_keys(
                            args, CompetitiveConfig, "decoder_competitive", seen)
                        config.decoder.competitivecrossattn = cls._copy_keys(
                            args, CompetitiveCrossAttnConfig, "decoder_competitivecrossattn", seen)
                elif fld.name == "encoder":
                    # same but for encoder
                    if safe_hasattr(args, "encoder"):
                        seen.add("encoder")
                        config.encoder = EncDecCompConfig(**args.encoder)
                        config.encoder.competitive = CompetitiveConfig(**args.encoder.competitive)
                    else:
                        config.encoder = cls._copy_keys(
                            args, EncDecCompConfig, "encoder", seen
                        )
                        config.encoder.competitive = cls._copy_keys(
                            args, CompetitiveConfig, "encoder_competitive", seen)
